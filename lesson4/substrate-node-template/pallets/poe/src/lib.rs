...
decl_storage! {
	...
	trait Store for Module<T: Trait> as PoeModule {
		...
		ProofsHash get(fn proofs_hash): map hasher(blake2_128_concat) Vec<u8> => (T::AccountId, T::BlockNumber, T::Moment, Option<Vec<u8>>);
		OwnedProofsHashList get(fn owned_proofs_hash_list): map hasher(blake2_128_concat) T::AccountId => Vec<Vec<u8>>;
	}
}
...
decl_event!(
		...
		Balance = BalanceOf<T>,
	{
		...
		ClaimCreatedWithNote(AccountId, Vec<u8>, Vec<u8>),
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		pub fn create_claim_with_note(origin, claim: Vec<u8>, note: Option<Vec<u8>>) -> dispatch::DispatchResult {
			...
			ProofsHash::<T>::insert(&claim, (sender.clone(), system::Module::<T>::block_number(), timestamp::Module::<T>::now(), note.clone()));

			if OwnedProofsHashList::<T>::contains_key(&sender) {
				let mut claims = OwnedProofsHashList::<T>::get(&sender);
				match claims.binary_search(&claim) {
					Ok(_) => (),
					Err(index) => claims.insert(index, claim.clone()),
				}
				OwnedProofsHashList::<T>::insert(&sender, claims);
			} else {
				let mut claims = Vec::<Vec<u8>>::new();
				claims.push(claim.clone());
				OwnedProofsHashList::<T>::insert(&sender, claims);
			}

			Self::deposit_event(RawEvent::ClaimCreatedWithNote(sender, claim, note.unwrap()));
